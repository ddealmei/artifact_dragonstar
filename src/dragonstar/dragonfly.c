/*
 * Shared Dragonfly functionality
 * Copyright (c) 2012-2016, Jouni Malinen <j@w1.fi>
 * Copyright (c) 2019, The Linux Foundation
 *
 * This software may be distributed under the terms of the BSD license.
 * See README for more details.
 */

#include <stdio.h>
#include <stdlib.h>

#include "common.h"
#include "const_time.h"
#include "crypto.h"
#include "crypto_bignum_static.h"
#include "dragonfly.h"


int dragonfly_suitable_group(int group, int ecc_only)
{
	/* Enforce REVmd rules on which SAE groups are suitable for production
	 * purposes: FFC groups whose prime is >= 3072 bits and ECC groups
	 * defined over a prime field whose prime is >= 256 bits. Furthermore,
	 * ECC groups defined over a characteristic 2 finite field and ECC
	 * groups with a co-factor greater than 1 are not suitable. Disable
	 * groups that use Brainpool curves as well for now since they leak more
	 * timing information due to the prime not being close to a power of
	 * two. */
	return group == 19; // || group == 20 || group == 21; 
    /* ||
		(!ecc_only &&
		 (group == 15 || group == 16 || group == 17 || group == 18));
    */
}


unsigned int dragonfly_min_pwe_loop_iter(int group)
{
	// if (group == 22 || group == 23 || group == 24) {
	// 	/* FFC groups for which pwd-value is likely to be >= p
	// 	 * frequently */
	// 	return 40;
	// }

	// if (group == 1 || group == 2 || group == 5 || group == 14 ||
	//     group == 15 || group == 16 || group == 17 || group == 18) {
	// 	/* FFC groups that have prime that is close to a power of two */
	// 	return 1;
	// }

	/* Default to 40 (this covers most ECC groups) */
	return 40;
}


int dragonfly_get_random_qr_qnr(const struct crypto_bignum_static *prime,
				struct crypto_bignum_static **qr,
				struct crypto_bignum_static **qnr)
{
	*qr = *qnr = NULL;
#ifdef DEBUG
	/* Instead of generating random qr / nqr, we generate them once
     * and for all, and hardcode them. This is only to remove randomness in our
     * analysis. Original code can be found commented bellow. */
	unsigned char qr_bin[] = { 0xe4, 0xf3, 0x30, 0x16, 0xe5, 0x3d, 0x3b, 0xec, 0x8d, 0x71, 0x69, 0x90, 0x47, 0xf1, 0x0f, 0x89, 0xa0, 0x37, 0x77, 0x1e, 0x5a, 0x66, 0x31, 0x3e, 0xf9, 0x43, 0x9f, 0x64, 0x42, 0x4a, 0x8b, 0xaf };
	unsigned char qnr_bin[] = { 0xb9, 0x81, 0x2f, 0x01, 0x90, 0x10, 0x05, 0x9c, 0x98, 0x02, 0xc0, 0x57, 0x1a, 0xdf, 0xb3, 0x21, 0xbe, 0x97, 0x29, 0xa3, 0x4e, 0xd1, 0xd7, 0xf7, 0xe3, 0xd4, 0xe8, 0xd4, 0x57, 0xf8, 0x9b, 0xb4 };

	*qr = crypto_bignum_static_init_set(qr_bin, 32, 32);
	*qnr = crypto_bignum_static_init_set(qnr_bin, 32, 32);
#else
	while (!(*qr) || !(*qnr)) {
		struct crypto_bignum_static *tmp;
		int res;

		tmp = crypto_bignum_static_init(crypto_bignum_static_size(prime));
		if (!tmp || crypto_bignum_static_rand(tmp, prime) < 0) {
			crypto_bignum_static_deinit(tmp, 0);
			break;
		}

		res = crypto_bignum_static_legendre(tmp, prime);
		if (res == 1 && !(*qr))
			*qr = tmp;
		else if (res == -1 && !(*qnr))
			*qnr = tmp;
		else
			crypto_bignum_static_deinit(tmp, 0);
	}
#endif
	if (*qr && *qnr)
		return 0;
	crypto_bignum_static_deinit(*qr, 0);
	crypto_bignum_static_deinit(*qnr, 0);
	*qr = *qnr = NULL;
	return -1;
}


static struct crypto_bignum_static*
dragonfly_get_rand_1_to_p_1(const struct crypto_bignum_static* prime)
{
	struct crypto_bignum_static* tmp, * pm1, * one;
	size_t prime_len = crypto_bignum_static_size(prime);

	tmp = crypto_bignum_static_init(prime_len);
	pm1 = crypto_bignum_static_init(prime_len);
	one = crypto_bignum_static_init_set((const uint8_t*) "\x01", 1, prime_len);
	if (!tmp || !pm1 || !one ||
	    crypto_bignum_static_sub(prime, one, pm1) < 0 ||
	    crypto_bignum_static_rand(tmp, pm1) < 0 ||
	    crypto_bignum_static_add(tmp, one, tmp) < 0) {
		crypto_bignum_static_deinit(tmp, 0);
		tmp = NULL;
	}

	crypto_bignum_static_deinit(pm1, 0);
	crypto_bignum_static_deinit(one, 0);
	return tmp;
}


int dragonfly_is_quadratic_residue_blind(struct crypto_ec *ec,
					 const u8 *qr, const u8 *qnr,
					 const struct crypto_bignum_static *val)
{
	struct crypto_bignum_static *r, *num, *qr_or_qnr = NULL;
	int check, res = -1;
	u8 qr_or_qnr_bin[DRAGONFLY_MAX_ECC_PRIME_LEN];
	const struct crypto_bignum_static *prime;
	size_t prime_len;
	unsigned int mask;

	prime = crypto_ec_get_prime_static(ec);
	prime_len = crypto_ec_prime_len(ec);

	/*
	 * Use a blinding technique to mask val while determining whether it is
	 * a quadratic residue modulo p to avoid leaking timing information
	 * while determining the Legendre symbol.
	 *
	 * v = val
	 * r = a random number between 1 and p-1, inclusive
	 * num = (v * r * r) modulo p
	 */
	r = dragonfly_get_rand_1_to_p_1(prime);
	if (!r)
		return -1;

	num = crypto_bignum_static_init(prime_len);
	if (!num ||
		crypto_bignum_static_mulmod(val, r, prime, num) < 0 ||
		crypto_bignum_static_mulmod(num, r, prime, num) < 0)
		goto fail;

	/*
	 * Need to minimize differences in handling different cases, so try to
	 * avoid branches and timing differences.
	 *
	 * If r is odd:
	 * num = (num * qr) module p
	 * LGR(num, p) = 1 ==> quadratic residue
	 * else:
	 * num = (num * qnr) module p
	 * LGR(num, p) = -1 ==> quadratic residue
	 *
	 * mask is set to !odd(r)
	 */
	mask = const_time_is_zero(crypto_bignum_static_is_odd(r));
	const_time_select_bin(mask, qnr, qr, prime_len, qr_or_qnr_bin);
	qr_or_qnr = crypto_bignum_static_init_set(qr_or_qnr_bin, prime_len, prime_len);
	if (!qr_or_qnr ||
		crypto_bignum_static_mulmod(num, qr_or_qnr, prime, num) < 0)
		goto fail;
	/* branchless version of check = odd(r) ? 1 : -1, */
	check = const_time_select_int(mask, -1, 1);

	/* Determine the Legendre symbol on the masked value */
	res = crypto_bignum_static_legendre(num, prime);
	if (res == -2) {
		res = -1;
		goto fail;
	}
	/* branchless version of res = res == check
	 * (res is -1, 0, or 1; check is -1 or 1) */
	mask = const_time_eq(res, check);
	res = const_time_select_int(mask, 1, 0);
fail:
	crypto_bignum_static_deinit(num, 1);
	crypto_bignum_static_deinit(r, 1);
	crypto_bignum_static_deinit(qr_or_qnr, 1);
	return res;
}


static int dragonfly_get_rand_2_to_r_1(struct crypto_bignum_static* val,
	const struct crypto_bignum_static* order)
{
	return crypto_bignum_static_rand(val, order) == 0 &&
		!crypto_bignum_static_is_zero(val) &&
		!crypto_bignum_static_is_one(val);
}


int dragonfly_generate_scalar(const struct crypto_bignum_static *order,
			      struct crypto_bignum_static *_rand,
			      struct crypto_bignum_static *_mask,
			      struct crypto_bignum_static *scalar)
{
	int count;

	/* Select two random values rand,mask such that 1 < rand,mask < r and
	 * rand + mask mod r > 1. */
	for (count = 0; count < 100; count++) {
		if (dragonfly_get_rand_2_to_r_1(_rand, order) &&
		    dragonfly_get_rand_2_to_r_1(_mask, order) &&
			crypto_bignum_static_addmod(_rand, _mask, order, scalar) == 0 &&
		    // crypto_bignum_add(_rand, _mask, scalar) == 0 &&
		    // crypto_bignum_mod(scalar, order, scalar) == 0 &&
		    !crypto_bignum_static_is_zero(scalar) &&
		    !crypto_bignum_static_is_one(scalar))
			return 0;
	}
#if DEBUG
	/* This should not be reachable in practice if the random number
	 * generation is working. */
	fprintf(stderr,
		   "dragonfly: Unable to get randomness for own scalar\n");
#endif
	return -1;
}


/* res = sqrt(val) */
int dragonfly_sqrt(struct crypto_ec *ec, const struct crypto_bignum_static *val,
		   struct crypto_bignum_static *res)
{
	const struct crypto_bignum_static *prime;
	struct crypto_bignum_static *tmp, *one;
	int ret = 0;
	u8 prime_bin[DRAGONFLY_MAX_ECC_PRIME_LEN];
	size_t prime_len;

	/* For prime p such that p = 3 mod 4, sqrt(w) = w^((p+1)/4) mod p */

	prime = crypto_ec_get_prime_static(ec);
	prime_len = crypto_ec_prime_len(ec);
	tmp = crypto_bignum_static_init(prime_len);
	one = crypto_bignum_static_init_uint(1, prime_len);

	if (crypto_bignum_static_to_bin(prime, prime_bin, sizeof(prime_bin),
				 prime_len) < 0 ||
	    (prime_bin[prime_len - 1] & 0x03) != 3 ||
	    !tmp || !one ||
	    /* tmp = (p+1)/4 */
	    crypto_bignum_static_add(prime, one, tmp) < 0 ||
	    crypto_bignum_static_rshift(tmp, 2, tmp) < 0 ||
	    /* res = sqrt(val) */
		crypto_bignum_static_exptmod(val, tmp, prime, res) < 0)
		ret = -1;

	crypto_bignum_static_deinit(tmp, 0);
	crypto_bignum_static_deinit(one, 0);
	return ret;
}
