...
{
    struct crypto_bignum_static* bn;
...
/* DIFF: takes one additional arg (can be avoided) */
struct crypto_bignum_static* crypto_bignum_static_init_set(const u8* buf, size_t buflen, size_t len)
{
    struct crypto_bignum_static* bn;
    u8* tmp;
    ...
    if (tmp != buf && tmp)
        free(tmp);
    return (struct crypto_bignum_static*) bn;
}
...
int crypto_bignum_static_rand(struct crypto_bignum_static* r, const struct crypto_bignum_static* m)
{
    ...
    } while (crypto_bignum_static_cmp(r, m) > 0);
    return 0;
}

/* Carefull, this retruns the carry */
int crypto_bignum_static_add(const struct crypto_bignum_static* a,
    const struct crypto_bignum_static* b, struct crypto_bignum_static* c)
{
...
static inline void static_mod_even(const struct crypto_bignum_static* a,
    const struct crypto_bignum_static* b, struct crypto_bignum_static* c)
{
    struct crypto_bignum_static* zero = crypto_bignum_static_init_uint(0, a->data_size);
    crypto_bignum_static_addmod(a, zero, b, c);
    crypto_bignum_static_deinit(zero, 0);
}


static inline void static_mod_odd(const struct crypto_bignum_static* a,
    const struct crypto_bignum_static* b, struct crypto_bignum_static* c)
{
    u64* tmp;
...
int crypto_bignum_static_mod(const struct crypto_bignum_static* a,
    const struct crypto_bignum_static* b, struct crypto_bignum_static* c)
{
    if (crypto_bignum_static_is_odd(b))
...
/* Carefull, this retruns the carry */
int crypto_bignum_static_sub(const struct crypto_bignum_static* a, const struct crypto_bignum_static* b, struct crypto_bignum_static* c)
{
    if (TEST_FAIL())
        return -1;
...
static int crypto_bignum_static_rshift1(const struct crypto_bignum_static* a, struct crypto_bignum_static* r) {
    u64* ap, * rp, t, c;
    int i;
    ...
    // if (crypto_bignum_static_is_zero(a)) {
...
int crypto_bignum_static_rshift(const struct crypto_bignum_static* a, int n, struct crypto_bignum_static* r) {
    ...
    for (size_t i = 0; i < n; i++)
        err |= crypto_bignum_static_rshift1(a, r);
    return err;
}
...
{
    struct crypto_bignum_static* tmp = NULL, *exp = NULL;
    int res = -2;
    unsigned int mask;
    ...
    if (a->meth->sub(p->data, (u64*)one_bn256, exp->data))
        goto fail;
    if (crypto_bignum_static_rshift1(exp, exp))
        goto fail;
    ...
    res = -1;
    mask = const_time_eq(crypto_bignum_static_is_one(tmp), 1);
    res = const_time_select_int(mask, 1, res);
    mask = const_time_eq(crypto_bignum_static_is_zero(tmp), 1);
    res = const_time_select_int(mask, 0, res);

fail:
    crypto_bignum_static_deinit(tmp, 1);
    crypto_bignum_static_deinit(exp, 1);
...
struct crypto_ec {
    struct crypto_bignum_static* prime;
    struct crypto_bignum_static* order;
    struct crypto_bignum_static* a;
    struct crypto_bignum_static* b;
...
void crypto_ec_deinit(struct crypto_ec* e)
{
    if (e == NULL)
        return;
    crypto_bignum_static_deinit(e->order, 0);
    crypto_bignum_static_deinit(e->prime, 0);
    crypto_bignum_static_deinit(e->a, 0);
    crypto_bignum_static_deinit(e->b, 0);
    os_free(e);
}
...
{
	return (const struct crypto_bignum_static *) e->a;
}


const struct crypto_bignum_static * crypto_ec_get_b_static(struct crypto_ec *e)
{
	return (const struct crypto_bignum_static *) e->b;
}
...
int crypto_ec_point_mul_static(struct crypto_ec *e,
    const struct crypto_ec_point* p, const struct crypto_bignum_static* b,
    struct crypto_ec_point* res)
{
    ...
    if (scalar != NULL && p->data != NULL && res_buf != NULL && point_buf != NULL) {
        ...
        crypto_bignum_static_to_bin(b, scalar, b->data_size, b->data_size);
...
struct crypto_bignum_static* crypto_ec_point_compute_y_sqr_static(
    struct crypto_ec *e, const struct crypto_bignum_static* x)
{
    struct crypto_bignum_static *ysqr;
    ...
    // (x^2 + a)*x + b % p
    crypto_bignum_static_sqrmod(x, e->prime, ysqr);
    crypto_bignum_static_addmod(ysqr, e->a, e->prime, ysqr);
    crypto_bignum_static_mulmod(ysqr, x, e->prime, ysqr);
    crypto_bignum_static_addmod(ysqr, e->b, e->prime, ysqr);
