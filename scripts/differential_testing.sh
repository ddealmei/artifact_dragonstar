...
echo "Comparing outputs of $n_passwords for OpenSSL and HaCl*"
...
[ -s $diff_file ] && echo "[-] Output not matching for legacy . Run \"cat $diff_file\" to get more details" || echo "[+] PASSED"
...
[ -s $diff_file ] && echo -e "[-] Output not matching for sswu . Run \"cat $diff_file\" to get more details" || echo -e "[+] PASSED sswu"